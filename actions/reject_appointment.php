...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update_stmt = $conn->prepare("UPDATE appointments SET status = 'rejected', updated_at = NOW() WHERE appointment_id = ?");
    ...
    if ($update_stmt->execute()) {
        ...
        $patient_msg = "Your appointment request has been rejected by Dr. $doctor_name. Please try booking another time.";
        create_notification($conn, $patient_id, $patient_msg);
        
        $_SESSION['success'] = "Appointment rejected";
    } else {
        $_SESSION['errors'] = ["Failed to reject appointment"];
    }
